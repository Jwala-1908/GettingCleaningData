...
findMeanStd <- fin_X[grepl("-mean()-", feat$V2, fixed = TRUE)|
...
for (i in 1:dim(fin_y)[1]) {
    fin_y$V1[i] = labData$Label[which(labData$Code==fin_y$V1[i])]
}
...
new_fin <- data.frame()
for (subj in unique(combined_fin$SubjectID)){
    for (ac in unique(combined_fin$Activity)){
        tmp <- c(subj, ac, newGen(combined_fin, subj, act))
